// backend/scripts/migrations/final_verification_fixed.mjs
import pg from 'pg';
  ...
  async verifyMigrationOnly() {
    try {
      ...
      entriesResult.rows.forEach((entry, index) => {
        const totalAmount = this.parseDecimal(entry.total_amount);
        const totalDebits = this.parseDecimal(entry.total_debits);
        const totalCredits = this.parseDecimal(entry.total_credits);
        
        console.log(`\n  Entry ${index + 1}: ${entry.reference_number}`);
        console.log(`    Description: ${entry.description}`);
        console.log(`    Amount: USh ${totalAmount.toFixed(2)}`);
        console.log(`    Debits: USh ${totalDebits.toFixed(2)}`);
        console.log(`    Credits: USh ${totalCredits.toFixed(2)}`);
      ...
      trialBalanceResult.rows.forEach(account => {
        const debits = this.parseDecimal(account.total_debits);
        const credits = this.parseDecimal(account.total_credits);
        const balance = debits - credits;
        console.log(`  ${account.account_code} ${account.account_name}: USh ${Math.abs(balance).toFixed(2)} (${balance > 0 ? 'Debit' : 'Credit'})`);
      });
      
      // Calculate totals
      const totalDebits = trialBalanceResult.rows.reduce((sum, row) => sum + this.parseDecimal(row.total_debits), 0);
      const totalCredits = trialBalanceResult.rows.reduce((sum, row) => sum + this.parseDecimal(row.total_credits), 0);
      ...
      const assets = trialBalanceResult.rows
        ...
        .reduce((sum, row) => {
          const debits = this.parseDecimal(row.total_debits);
          const credits = this.parseDecimal(row.total_credits);
          return sum + (debits - credits); // Assets have debit balances
        }, 0);
      ...
      const liabilities = trialBalanceResult.rows
        ...
        .reduce((sum, row) => {
          const debits = this.parseDecimal(row.total_debits);
          const credits = this.parseDecimal(row.total_credits);
          return sum + (credits - debits); // Liabilities have credit balances
        }, 0);
      ...
      const equity = trialBalanceResult.rows
        ...
        .reduce((sum, row) => {
          const debits = this.parseDecimal(row.total_debits);
          const credits = this.parseDecimal(row.total_credits);
          return sum + (credits - debits); // Equity has credit balances
        }, 0);
      ...
      const revenue = trialBalanceResult.rows
        ...
        .reduce((sum, row) => {
          const debits = this.parseDecimal(row.total_debits);
          const credits = this.parseDecimal(row.total_credits);
          return sum + (credits - debits); // Revenue has credit balances
        }, 0);
      ...
      const expenses = trialBalanceResult.rows
        ...
        .reduce((sum, row) => {
          const debits = this.parseDecimal(row.total_debits);
          const credits = this.parseDecimal(row.total_credits);
          return sum + (debits - credits); // Expenses have debit balances
        }, 0);
      
      const netProfitLoss = revenue - expenses;
      const finalEquity = equity + netProfitLoss; // Equity after P&L
      
      console.log('\n💰 FINANCIAL POSITION (Migration Only):');
      console.log(`  Assets: USh ${assets.toFixed(2)}`);
      console.log(`  Liabilities: USh ${liabilities.toFixed(2)}`);
      console.log(`  Opening Equity: USh ${equity.toFixed(2)}`);
      console.log(`  Revenue: USh ${revenue.toFixed(2)}`);
      ...
      console.log(`  Final Equity: USh ${finalEquity.toFixed(2)}`);
      
      // Check balance sheet equation
      const isBalanced = Math.abs(assets - (liabilities + finalEquity)) < 0.01;
      
      console.log(`\n⚖️  BALANCE SHEET EQUATION:`);
      console.log(`  Assets (${assets.toFixed(2)}) = Liabilities (${liabilities.toFixed(2)}) + Equity (${finalEquity.toFixed(2)})`);
      ...
      console.log('    Revenue: USh 9,035.00');
      console.log('    Expenses + COGS: USh 83,720.00 (82,425 + 1,295)');
      console.log('    Net Loss: USh -74,685.00');
      console.log('    Final Equity: USh 3,534,411.32');
      ...
      console.log(`    Revenue: USh ${revenue.toFixed(2)}`);
      console.log(`    Expenses: USh ${expenses.toFixed(2)}`);
      console.log(`    Net Loss: USh ${netProfitLoss.toFixed(2)}`);
      console.log(`    Final Equity: USh ${finalEquity.toFixed(2)}`);
      ...
      const report = {
        ...
        journal_entries: entriesResult.rows.map(entry => ({
          ...entry,
          total_amount: this.parseDecimal(entry.total_amount),
          total_debits: this.parseDecimal(entry.total_debits),
          total_credits: this.parseDecimal(entry.total_credits)
        })),
        trial_balance: trialBalanceResult.rows.map(account => ({
          ...account,
          total_debits: this.parseDecimal(account.total_debits),
          total_credits: this.parseDecimal(account.total_credits)
        })),
        financial_position: {
          assets: assets,
        ...
        comparison_with_audit: {
          ...
          liabilities_match: Math.abs(liabilities - 1000) < 0.01,
          equity_match: Math.abs(finalEquity - 3534411.32) < 0.01,
          notes: 'Small differences due to database (9,030) vs audit (9,035) revenue'
        }
      };
