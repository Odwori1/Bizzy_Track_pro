// backend/scripts/migrations/create_opening_journal_entries_final.mjs
import pg from 'pg';
...
class JournalEntryCreator {
  ...
  async createJournalEntry(entryData) {
    try {
      ...
      // Generate unique reference ID for this entry
      const entryReferenceId = `${this.migrationBatchId}_entry_${entryData.entry_number}`;
      ...
      const journalEntryResult = await this.client.query(
        ...
        [
          ...
          entryReferenceId, // UNIQUE for each entry
          totalDebits,
          'posted',
          this.userId,
          this.migrationBatchId,
          'migration_script',
          `entry_${entryData.entry_number}`
        ]
      );
  ...
  async executeMigration() {
    try {
      ...
      // Create migration audit record
      console.log('📝 Creating migration audit record...');
      ...
      const results = {
        ...
        created_entries: this.createdEntries.map(e => ({
          ...
          total_amount: e.journal_entry.total_amount
        })),
      ...
      if (verification.trial_balance.length > 0) {
...
async function main() {
  try {
    ...
    if (!businessId || !userId) {
      console.error('Usage: node create_opening_journal_entries_final.mjs <business_id> <user_id>');
