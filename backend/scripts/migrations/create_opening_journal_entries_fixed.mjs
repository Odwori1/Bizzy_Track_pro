// backend/scripts/migrations/create_opening_journal_entries_fixed.mjs
import pg from 'pg';
...
class JournalEntryCreator {
  ...
  async createJournalEntry(entryData) {
    try {
      ...
      // Create journal entry - FIXED: migration_source_id is now VARCHAR
      const journalEntryResult = await this.client.query(
        ...
        [
          ...
          'migration_script',
          `entry_${entryData.entry_number}` // Now this can be text
        ]
      );
...
async function main() {
  try {
    ...
    if (!businessId || !userId) {
      console.error('Usage: node create_opening_journal_entries_fixed.mjs <business_id> <user_id>');
