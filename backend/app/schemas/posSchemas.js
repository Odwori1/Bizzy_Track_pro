...
export const createPOSTransactionSchema = Joi.object({
  ...
  items: Joi.array().items(
    Joi.object({
      ...
      booking_id: Joi.string().uuid().optional().allow(null),  // ✅ NEW: For equipment hire bookings
  ...
  for (let i = 0; i < value.items.length; i++) {
    ...
    if (item.item_type === 'equipment_hire') {
      if (!item.equipment_id) {
        return helpers.error('any.invalid', {
          message: `items[${i}].equipment_id is required for equipment_hire items`
        });
      }
      // Optionally validate booking_id for equipment hire
      if (!item.booking_id) {
        return helpers.error('any.invalid', {
          message: `items[${i}].booking_id is recommended for equipment_hire items`
        });
      }
    }
  }
