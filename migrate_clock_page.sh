...
echo "=== MIGRATING CLOCK PAGE TO USE STORE ==="

# Create backup
cp frontend/src/app/dashboard/management/workforce/timesheets/clock/page.tsx \
   frontend/src/app/dashboard/management/workforce/timesheets/clock/page.tsx.backup

# Create the migrated file
cat > frontend/src/app/dashboard/management/workforce/timesheets/clock/page.tsx << 'MIGRATED'
'use client';
...
export default function TimeClockPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      await Promise.all([
        ...
        actions.fetchClockEvents(undefined, 10)
      ]);
  ...
  // Find events for an employee by matching employee_id
  const findEventsForEmployee = (employeeId: string) => {
    const employee = employees.find(emp => emp.id === employeeId);
    if (!employee) return [];

    // Try to match by employee_id field in events
    return clockEvents.filter(event =>
      event.employee_id === employee.employee_id ||
      event.staff_profile_id === employee.employee_id ||
      event.staff_profile_id === employee.id
    );
  };

  const getCurrentStatus = (employeeId: string) => {
    const employeeEvents = findEventsForEmployee(employeeId);
    if (employeeEvents.length === 0) return 'out';
    ...
    if (lastEvent.event_type === 'clock_in') return 'in';
    if (lastEvent.event_type === 'break_start') return 'break';
    if (lastEvent.event_type === 'break_end') return 'in';
    return 'out';
  };
...
echo "✅ Clock page migrated to use unified store"
echo "✅ Backup saved to page.tsx.backup"

# Verify the migration
echo ""
echo "=== VERIFICATION ==="
echo "Check imports (should use store):"
grep -n "useUnifiedEmployeesStore" frontend/src/app/dashboard/management/workforce/timesheets/clock/page.tsx
echo ""
echo "Check for old hook (should not exist):"
grep -n "useUnifiedEmployees" frontend/src/app/dashboard/management/workforce/timesheets/clock/page.tsx || echo "✓ Old hook removed"
