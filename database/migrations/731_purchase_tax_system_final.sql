-- 731_purchase_tax_system_final.sql
-- Phase 7: Purchase Tax Integration - PostgreSQL 12+ Compatible
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_orders' AND column_name = 'tax_rate') THEN
        ALTER TABLE purchase_orders ADD COLUMN tax_rate DECIMAL(5,2) DEFAULT 20.00;
    END IF;
    
    -- Add wht_applicable if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_orders' AND column_name = 'wht_applicable') THEN
        ALTER TABLE purchase_orders ADD COLUMN wht_applicable BOOLEAN DEFAULT false;
    END IF;
    
    -- Add wht_rate if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_orders' AND column_name = 'wht_rate') THEN
        ALTER TABLE purchase_orders ADD COLUMN wht_rate DECIMAL(5,2);
    END IF;
    
    -- Add wht_amount if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_orders' AND column_name = 'wht_amount') THEN
        ALTER TABLE purchase_orders ADD COLUMN wht_amount DECIMAL(15,2);
    END IF;
    
    -- Add tax_calculation if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'purchase_orders' AND column_name = 'tax_calculation') THEN
        ALTER TABLE purchase_orders ADD COLUMN tax_calculation JSONB;
    END IF;
...
-- Add indexes for new columns (CREATE INDEX IF NOT EXISTS works in all versions)
...
DO $$ 
BEGIN
    -- Add tin_verified if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'suppliers' AND column_name = 'tin_verified') THEN
        ALTER TABLE suppliers ADD COLUMN tin_verified BOOLEAN DEFAULT false;
    END IF;
    
    -- Add tin_verified_at if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'suppliers' AND column_name = 'tin_verified_at') THEN
        ALTER TABLE suppliers ADD COLUMN tin_verified_at TIMESTAMPTZ;
    END IF;
    
    -- Add tin_verification_status if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'suppliers' AND column_name = 'tin_verification_status') THEN
        ALTER TABLE suppliers ADD COLUMN tin_verification_status VARCHAR(20) DEFAULT 'pending';
    END IF;
    
    -- Add compliance_score if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'suppliers' AND column_name = 'compliance_score') THEN
        ALTER TABLE suppliers ADD COLUMN compliance_score DECIMAL(5,2) DEFAULT 100;
    END IF;
    
    -- Add risk_level if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'suppliers' AND column_name = 'risk_level') THEN
        ALTER TABLE suppliers ADD COLUMN risk_level VARCHAR(20) DEFAULT 'low';
    END IF;
    
    -- Add last_compliance_check if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'suppliers' AND column_name = 'last_compliance_check') THEN
        ALTER TABLE suppliers ADD COLUMN last_compliance_check TIMESTAMPTZ;
    END IF;
END $$;

-- Add check constraints (drop existing first, then add new)
