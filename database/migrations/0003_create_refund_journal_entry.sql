...
BEGIN
    ...
    IF v_original_transaction_type = 'POS' THEN
        ...
        WHERE pt.id = v_original_transaction_id;
    ...
    IF v_refund_method = 'CREDIT_NOTE' THEN
        SELECT coa.id INTO v_credit_note_account_id
    ...
    SELECT coa.id INTO v_sales_returns_account_id
    FROM chart_of_accounts coa
    WHERE coa.business_id = v_business_id
      AND coa.account_code = '4150'
      AND coa.is_active = true;
    ...
    IF v_discount_refunded > 0 THEN
        SELECT coa.id INTO v_discount_account_id
        FROM chart_of_accounts coa
        WHERE coa.business_id = v_business_id
          AND coa.account_code = v_discount_account_code
          AND coa.is_active = true;
        ...
        IF v_discount_account_id IS NULL THEN
            ...
            FROM chart_of_accounts coa
            WHERE coa.business_id = v_business_id
              AND coa.account_code = '4110'
              AND coa.is_active = true;
        END IF;
    END IF;

    IF v_tax_refunded > 0 THEN
        SELECT coa.id INTO v_tax_account_id
        FROM chart_of_accounts coa
        WHERE coa.business_id = v_business_id
          AND coa.account_code = '2120'
          AND coa.is_active = true;
    END IF;
    ...
    IF v_restock_fee > 0 THEN
        SELECT coa.id INTO v_restock_fee_account_id
        FROM chart_of_accounts coa
        WHERE coa.business_id = v_business_id
          AND coa.account_code = '4160'
          AND coa.is_active = true;
    END IF;
    ...
    -- LINE 4: Credit COGS (reverse original COGS debit)
    ...
    -- LINE 5: Debit Inventory (restore inventory asset)
    ...
    -- LINE 6: Credit Restock Fee Revenue
