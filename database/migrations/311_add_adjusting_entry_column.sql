-- Migration 311: Add adjusting entry column to journal entries
-- Date: 2026-01-12
-- Purpose: Add is_adjusting_entry column for historical depreciation entries

BEGIN;

-- Add is_adjusting_entry column to journal_entries if it doesn't exist
ALTER TABLE journal_entries 
ADD COLUMN IF NOT EXISTS is_adjusting_entry BOOLEAN DEFAULT false;

-- Add is_adjusting_entry column to journal_entry_lines if it doesn't exist
ALTER TABLE journal_entry_lines 
ADD COLUMN IF NOT EXISTS is_adjusting_entry BOOLEAN DEFAULT false;

-- Update the post_historical_depreciation function to not use the column
-- (or we could keep it, but let's update to not require it for now)
    ...
    WHILE v_current_date <= LEAST(p_as_of_date, CURRENT_DATE) LOOP
        ...
        IF NOT EXISTS (
            ...
            IF v_depreciation_amount > 0 THEN
                ...
                -- REMOVED is_adjusting_entry column
