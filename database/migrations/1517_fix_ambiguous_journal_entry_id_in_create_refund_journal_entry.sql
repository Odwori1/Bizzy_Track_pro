-- Migration 1517: Fix ambiguous column reference in create_refund_journal_entry()
-- Root cause: the function's OUT parameter `journal_entry_id` (from RETURNS TABLE)
-- is a PL/pgSQL variable in scope for the whole function body. Migration 1515's
-- new balance-assertion query referenced journal_entry_lines.journal_entry_id
-- unqualified, colliding with that variable under the default
-- plpgsql.variable_conflict = error setting. This blocked ALL refund approvals
-- since 1515 shipped — every call to create_refund_journal_entry() failed with
-- "column reference journal_entry_id is ambiguous" at the balance-check step,
-- after the journal entry header and all lines had already been inserted in
-- the same transaction (rolled back cleanly by the surrounding trigger, so no
-- partial data was left behind — confirmed via live test, refund stayed PENDING).
...
BEGIN
    ...
    -- FIX (1517): qualify journal_entry_lines.journal_entry_id with table alias
    -- `jel` so it can no longer be confused with this function's own
    -- journal_entry_id OUT parameter/variable.
    SELECT
    ...
    FROM journal_entry_lines jel WHERE jel.journal_entry_id = v_journal_entry_id;
