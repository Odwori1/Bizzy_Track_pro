-- ============================================================================
-- FIX EXPENSE ACCOUNTING WORKFLOW - CORRECTED VERSION
-- ============================================================================
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'FIXING EXPENSE ACCOUNTING WORKFLOW';
    RAISE NOTICE '========================================';
END;
$$;

-- ============================================================================
-- PART 1: FIX EXISTING EXPENSES (Using system user or NULL)
-- ============================================================================
DO $$
DECLARE
...
BEGIN
    ...
    -- Try to get a system user, or use NULL
    ...
    FOR v_expense_record IN (
        SELECT 
            ...
            e.expense_date,
            ec.account_code,
            e.created_by  -- Use the original expense creator
        FROM expenses e
        ...
        -- Use original creator or system user
        v_system_user_id := COALESCE(v_expense_record.created_by, v_system_user_id);
        ...
        INSERT INTO journal_entries (
            ...
            v_system_user_id,  -- Use valid user ID
            NOW()
...
-- ============================================================================
-- PART 2: UPDATE process_expense_payment FUNCTION (Corrected)
-- ============================================================================
DO $$
BEGIN
    RAISE NOTICE '';
...
DROP FUNCTION IF EXISTS process_expense_payment(UUID, VARCHAR, UUID);
...
BEGIN
    ...
    -- Only process unpaid or approved expenses
...
-- ============================================================================
-- PART 3: CREATE EXPENSE APPROVAL FUNCTION (Corrected)
-- ============================================================================
DO $$
BEGIN
    RAISE NOTICE '';
...
BEGIN
    ...
    -- Check if already approved or has accounting
...
DECLARE
    ...
    v_expenses_with_payment INTEGER;
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    ...
    -- Count fixed expenses (now have recognition)
    ...
    -- Count expenses with payment entries
    SELECT COUNT(*) INTO v_expenses_with_payment
    FROM expenses e
    WHERE e.business_id = 'ac7de9dd-7cc8-41c9-94f7-611a4ade5256'
        AND e.status = 'paid'
        AND EXISTS (
    ...
    RAISE NOTICE 'Expense Accounting Status:';
    RAISE NOTICE '  Total paid expenses: %', v_total_paid_expenses;
    RAISE NOTICE '  With expense recognition: %', v_fixed_count;
    RAISE NOTICE '  With payment entries: %', v_expenses_with_payment;
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Checking Profit & Loss impact...';
    
    -- Show expense account totals
    SELECT 
        ca.account_name,
        ca.account_code,
        SUM(CASE WHEN jel.line_type = 'debit' THEN jel.amount ELSE -jel.amount END) as net_amount
    ...
    GROUP BY ca.account_name, ca.account_code
