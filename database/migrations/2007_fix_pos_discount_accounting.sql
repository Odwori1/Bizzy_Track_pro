...
DECLARE
    ...
    v_customer_type      VARCHAR(50);      -- drives tax account selection
    v_receiving_id       UUID;
    v_revenue_id         UUID;
    v_tax_id             UUID;
    v_discount_id        UUID;
...
BEGIN
    ...
    -- ── TAX ACCOUNT — data-driven by customer type ────────────────────────
    -- individual / walk-in / NULL → VAT  → 2120 Sales Tax Payable
    -- company                    → WHT  → 2130 WHT Payable
    --
    -- Fallback chain within each type ensures we always find an account:
    --   company:    try 2130 first, then 2120
    --   individual: try 2120 first, then 2130
    -- If neither exists the tax line is skipped (v_tax_id stays NULL)
    ...
    IF v_customer_type = 'company' THEN
        ...
        LIMIT 1;
    ELSE
        -- individual, walk-in, or any unrecognised type → VAT
        SELECT id INTO v_tax_id FROM chart_of_accounts
        WHERE business_id = v_business_id
    ...
    IF v_tax_id IS NULL AND v_net_tax > 0 THEN
        RAISE WARNING
            '[JE] No tax account (2120/2130) found for business % — tax line skipped for %',
    ...
    -- LINE 3: Tax CR — account determined by customer type
    ...
    -- LINE 4: Discount DR contra-revenue
    IF v_discount_amount > 0 THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE proname = 'create_journal_entry_for_pos_transaction_fixed'
          AND prosrc ILIKE '%v_customer_type%'
          AND prosrc ILIKE '%2120%'
          AND prosrc ILIKE '%2130%'
    ) THEN
        RAISE EXCEPTION 'Tax routing fix not detected in function body.';
    END IF;
    RAISE NOTICE '✓ Tax account routing: individual→2120 (VAT), company→2130 (WHT)';
    RAISE NOTICE '✓ COGS fix confirmed: v_product_count, v_total_cogs (two distinct variables)';
END $$;
