...
--
-- FIX NOTE: All standalone RAISE NOTICE calls are wrapped in DO $$ ... $$
-- blocks. RAISE is a PL/pgSQL-only statement and is not valid as a bare
-- top-level SQL statement, which is what caused every "syntax error at or
-- near RAISE" in the previous run.
-- ============================================================================
...
DO $$ BEGIN RAISE NOTICE '✅ Section 1 complete: Broken policies fixed'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 2 complete: RLS enabled on all 24 tables'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 3 complete: discount_allocation_lines isolation fixed'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 4 complete: All sequence generators fixed with advisory locks'; END $$;
...
-- An existing version of this function has different parameter/OUT-column
-- names for the same signature. CREATE OR REPLACE cannot rename parameters,
-- so the old version must be dropped first (per Postgres' own HINT).
DROP FUNCTION IF EXISTS post_monthly_depreciation_fixed(UUID, INTEGER, INTEGER, UUID);
...
DO $$ BEGIN RAISE NOTICE '✅ Section 6 complete: SECURITY DEFINER functions fixed with business_id filters'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 7 complete: Duplicate policies removed'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 8 complete: missing_ok standardized to true for consistency'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 9 complete: FORCE RLS enabled on all RLS-enabled tables'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 10 complete: Indexes created for all 24 tables'; END $$;
...
DO $$ BEGIN RAISE NOTICE '✅ Section 12 complete: Verification test queries documented'; END $$;
