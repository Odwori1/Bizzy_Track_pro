-- 731_complete_purchase_tax_system_fixed.sql
-- Phase 7: Complete Purchase Tax Integration - FIXED VERSION
-- Created: February 15, 2026

BEGIN;

-- ============================================================================
-- PART 1: ADD COLUMNS FIRST (NO GENERATED COLUMNS YET)
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1.1 First add all regular columns to purchase_orders
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 1.3 NOW add the GENERATED column that depends on tax_amount
-- ----------------------------------------------------------------------------
ALTER TABLE purchase_orders 
...
-- ----------------------------------------------------------------------------
-- 1.4 Add compliance fields to suppliers table
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 1.5 Add purchase_order_id to transaction_taxes if not exists
-- ----------------------------------------------------------------------------
DO $$ 
BEGIN
    IF NOT EXISTS (
...
CREATE INDEX IF NOT EXISTS idx_purchase_tax_credits_tax_type ON purchase_tax_credits(tax_type_code);
...
CREATE INDEX IF NOT EXISTS idx_import_duty_country ON import_duty_calculations(country_of_origin);
...
CREATE INDEX IF NOT EXISTS idx_supplier_tin_log_verified_at ON supplier_tin_verification_log(verified_at);
...
CREATE INDEX IF NOT EXISTS idx_purchase_wht_submitted ON purchase_wht_certificates(submitted_to_ura) WHERE submitted_to_ura = false;
...
-- ----------------------------------------------------------------------------
-- 5.3 Function to update tax credit status
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    AND column_name IN ('tax_amount', 'tax_rate', 'wht_applicable', 'wht_rate', 'wht_amount', 'tax_calculation', 'subtotal');
