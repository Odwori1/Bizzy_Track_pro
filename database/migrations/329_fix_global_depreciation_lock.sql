-- Migration 329: Fix Global Depreciation Lock Bug
-- Date: January 20, 2026
-- Fixes: post_monthly_depreciation_partial function prevents posting depreciation
--        for ANY asset if ANY other asset already has depreciation for that period
...
DECLARE
    ...
    v_asset_has_depreciation BOOLEAN; -- Renamed for clarity
BEGIN
    v_depreciation_date := MAKE_DATE(p_year, p_month, 1);

    -- ✅✅✅ CRITICAL FIX: REMOVED GLOBAL CHECK AT BEGINNING
    -- Old buggy code (commented out):
    /*
    -- Check if depreciation already posted for this period (GLOBAL CHECK - BUG!)
    SELECT EXISTS (
    ...
    */
    ...
    FOR v_asset IN (
        ...
        BEGIN
            -- ✅✅✅ CRITICAL FIX: Check if THIS ASSET already has depreciation for this period
            SELECT EXISTS (
                SELECT 1 FROM asset_depreciations ad
            ...
            -- If asset already has depreciation for this period, skip it
            ...
            IF v_depreciation_amount > 0 THEN
                ...
                -- Create depreciation record (without created_by column - already fixed)
