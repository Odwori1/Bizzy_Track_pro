...
DECLARE
    ...
    v_cogs_id            UUID;
    v_inventory_id       UUID;
    v_product_count      INTEGER := 0;      -- restored: needed for COUNT(*)
...
BEGIN
    ...
    -- COGS: COUNT into v_product_count, SUM into v_total_cogs
    SELECT
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE proname = 'create_journal_entry_for_pos_transaction_fixed'
          AND prosrc ILIKE '%INTO v_product_count, v_total_cogs%'
    ) THEN
        RAISE EXCEPTION 'COGS fix not detected in function body — check deployment.';
    END IF;
    RAISE NOTICE '✓ COGS fix verified: COUNT → v_product_count, SUM → v_total_cogs';
END $$;
