-- File: database/migrations/118_fix_pos_service_count_description.sql
BEGIN;

DO $$
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'FIXING POS SERVICE COUNT BUG';
    RAISE NOTICE '========================================';
END;
$$;

-- ============================================================================
-- PART 1: CREATE CORRECTED FUNCTION
-- ============================================================================
DO $$
BEGIN
...
-- ============================================================================
-- PART 2: FIX EXISTING WRONG DESCRIPTIONS
-- ============================================================================
DO $$
DECLARE
...
BEGIN
    ...
    -- Fix service transactions showing "0 items" or wrong counts
    UPDATE journal_entries je
    SET description = 
        CASE 
            WHEN service_count > 0 AND product_count = 0 THEN
                'POS Sale: ' || pt.transaction_number || 
                ' (' || pt.payment_method || ', ' || 
                service_count || ' service' || 
                CASE WHEN service_count != 1 THEN 's' ELSE '' END || ')'
            WHEN product_count > 0 AND service_count = 0 THEN
                'POS Sale: ' || pt.transaction_number || 
                ' (' || pt.payment_method || ', ' || 
                product_count || ' item' || 
                CASE WHEN product_count != 1 THEN 's' ELSE '' END || ')'
            ELSE
                'POS Sale: ' || pt.transaction_number || 
                ' (' || pt.payment_method || ', ' || 
                (product_count + service_count) || ' items)'
        END
    FROM (
        SELECT 
            je.id as journal_id,
    ...
    WHERE je.id = pt.journal_id
        AND (
            je.description LIKE '%0 items%' OR
            je.description NOT LIKE '%service%' AND pt.service_count > 0
        )
    RETURNING je.id INTO v_fixed_count;
